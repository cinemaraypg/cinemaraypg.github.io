...
var NodeMaterialBlock = /** @class */ (function () {
     ...
     */
    NodeMaterialBlock.prototype.bind = function (effect, nodeMaterial, mesh) {
        // Do nothing
    };
     ...
     */
    NodeMaterialBlock.prototype.registerInput = function (name, type, isOptional, target) {
        if (isOptional === void 0) { isOptional = false; }
        var point = new NodeMaterialConnectionPoint(name, this);
        point.type = type;
     ...
     * @returns the current block
     */
    NodeMaterialBlock.prototype.registerOutput = function (name, type, target) {
        var point = new NodeMaterialConnectionPoint(name, this);
        point.type = type;
        if (target) {
            point.target = target;
     ...
     * @param useInstances specifies that instances should be used
     */
    NodeMaterialBlock.prototype.prepareDefines = function (mesh, nodeMaterial, defines, useInstances) {
        if (useInstances === void 0) { useInstances = false; }
        // Do nothing
    };
    /**
    ...
    NodeMaterialBlock.prototype._processBuild = function (block, state, input, activeBlocks) {
        ...
        if (localBlockIsFragment && (((block.target & input.target) === 0) ||
            (this.target !== NodeMaterialBlockTargets.VertexAndFragment && otherBlockWasGeneratedInVertexShader))) { // context switch! We need a varying
